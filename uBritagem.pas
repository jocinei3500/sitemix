...
type
  ...
  private
    FIntervalSaveRemote:integer;//a cada 10 segundos é cadastrada no banco de dado remoto.=
    FIntervalAvgProd:integer;//so inicia o cálculo de produção após 5 segundos.
...
begin
  ...
  try
    ...
    if mctPLC.ReadHoldingRegisters(tag_hm_total_br,1, Data)then//horímetro total britagem
      begin
        dHmTotalBR:=Data[0];
        edEnergiaUsina.Caption:=FloatToStr(dHmTotalBR);
      end;

      if mctPLC.ReadHoldingRegisters(tag_hm_hora_br,1, Data)then//horímetro parcial hora britagem
      begin
        dHmHoraBR:=Data[0];
      end;

      if mctPLC.ReadHoldingRegisters(tag_hm_mm_br,1, Data)then//horímetro parcial minutos britagem
      begin
        dHmMmBR:=Data[0];
      ...
      begin
        ...
        if dmData.qCadProdLocal.RecordCount>0 then
          begin
            ...
            if FProducao_po<0 then FProducao_po:=0;//caso o valor de menor que 0 então fica 0.
...
procedure TfrBritagem.buscaConfig;//Busca configurações salvas no banco de dados.
var
  i:integer;
begin
  ...
  dmData.zQueryX.SQL.Text:='SELECT * FROM config_tags where nome = :nome';//busca TAG do homímetro da britagem
